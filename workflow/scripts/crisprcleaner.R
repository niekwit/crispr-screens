...
counts <- read.delim(snakemake@input[["counts"]], header = TRUE)
...
all.samples <- c(
  str_split(test.name, ",")[[1]],
  str_split(control.name, ",")[[1]]
)
counts <- counts %>%
...
if (str_detect(control.name, ",")) {
  ...
  control.columns <- which(
    colnames(counts) %in% strsplit(control.name, ",")[[1]]
  )
} else {
...
if (!library.name %in% available.libs) {
  ...
  if (!all(required.columns %in% colnames(full.annotations))) {
    stop(
      "Library file does not contain all required columns: ",
      paste(
        setdiff(required.columns, colnames(full.annotations)),
        collapse = ", "
      )
    )
  }
...
# This is important for CRISPRcleanR to work properly
if (!all(full.annotations$GENES == counts$gene)) {
  stop("Gene names in counts and annotations do not match.")
}
if (!all(full.annotations$CODE == counts$sgRNA)) {
...
print(paste(
  "Normalising raw counts and computing sgRNA log fold-changes against",
  control.name,
  "..."
))
print(paste(
  "Filtering sgRNAs for minimum number of reads in control:",
  min_reads
))
...
if (nrow(normANDfcs$logFCs) == 0) {
  stop(
    "ccr.NormfoldChanges command returned an empty data frame.\n 
       Please check the library annotation file or min_reads value."
  )
}

# Remove sgRNAs from full.annotations that are not in normANDfcs$logFCs$sgRNA
full.annotations <- full.annotations[
  rownames(full.annotations) %in% normANDfcs$logFCs$sgRNA,
]
...
correctedFCs <- ccr.GWclean(gwSortedFCs, display = FALSE, label = comparison)
...
correctedCounts <- ccr.correctCounts(
  comparison,
...
BAGEL_nonEssential_sgRNAs <- ccr.genes2sgRNAs(
  full.annotations,
  BAGEL_nonEssential
)
...
roc.plot <- ggplot(
  ...
  labs(
    title = paste0("ROC curve: gene and sgRNA level\n(", comparison, ")"),
    x = "TNR",
    y = "Recall"
  ) +
  theme_cowplot(18) +
...
ggsave(filename = snakemake@output[["roc"]], roc.plot, width = 8, height = 6)

### PR curves
# Calculate PR curve data for gene and sgRNA level
sgRNA_level_PR <- ccr.PrRc_Curve(
  FCs,
  BAGEL_essential_sgRNAs,
  BAGEL_nonEssential_sgRNAs,
  display = FALSE
...
pr.plot <- ggplot(df, aes(x = recall, y = precision, color = class)) +
  geom_line(linewidth = 1) +
  labs(
    title = paste0("PR curve: gene and sgRNA level\n(", comparison, ")"),
    x = "Recall",
    y = "Precision"
  ) +
  theme_cowplot(18) +
  annotate(
    ...
    label = paste(
      "AUC = ",
      round(gene_level_PR$AUC, 3),
      " (gene), ",
      round(sgRNA_level_PR$AUC, 3),
      " (sgRNA)"
    )
  ) +
  theme(legend.position = c(0.025, 0.15), legend.title = element_blank())

ggsave(snakemake@output[["pr"]], pr.plot, width = 8, height = 6)
...
Recall_scores <- ccr.VisDepAndSig(
  FCsprofile = geneFCs,
